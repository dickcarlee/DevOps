...
class DevopsDUProjectManageTest(BaseCase):
    ...
    def test_page_display(self):
        ...
        self.click(self.page.menuProjectManage)
        self.assert_text('项目管理', self.page.header)
    ...
    def test_table_order_desc(self):
        ...
        self.click(self.page.menuProjectManage)
        try:
            time0 = self.get_text(self.page.tableTrProjectManageTime0)
            time1 = self.get_text(self.page.tableTrProjectManageTime1)
            assert time0 > time1
        except:
            time0 = self.get_text(self.page.tableTrProjectManageTime0)
    ...
    def test_new_config(self):
        DevopsLoginPage().login(self)
        self.click(self.page.menuProjectManage)
        self.click(self.page.btnAddProject)
        _now = str(int(time.time()))
        for i in range(2):
            time.sleep(0.5)
            _element = self.find_visible_elements(self.page.inputModalAutoComplete)[-1]
            _element.click()
            _element.send_keys(Keys.ARROW_UP)
            _element.send_keys(Keys.ENTER)
        self.update_text_value(self.page.modalInputProjectName, 'Name' + _now)
        self.click(self.page.btnModalAccept)
        self.assert_element_not_visible(self.page.modalInputProjectName)
